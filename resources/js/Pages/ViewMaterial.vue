<template>
  ...
  <section class="p-4 md:p-6 max-w-screen-2xl mx-auto transition-all duration-500">

    <div style="background: url(`.././${material.image_src}`) center center/cover"
      class="rounded-2xl overflow-hidden transition-all duration-200">

      <div
        class="group bg-gradient-to-r from-black/60 via-black/50 to-black/40 backdrop-blur-lg flex flex-col md:flex-row justify-between gap-2 p-3 transition-all duration-500">
        <!-- title and subject wrapper -->
        <div class="flex-grow flex flex-col text-white pt-2 px-2 md:px-4">
          <!-- title  -->
          <h2 class="title mb-2 text-xl font-semibold md:text-2xl md:font-bold capitalize leading-tight">
            {{material.title}}
          </h2>
          <!-- subject -->
          <p class="subject mb-3 text-sm font-medium uppercase leading-tight">
            {{ material.subject }}
          </p>

          <!-- username -->
          <div
            class="mt-auto max-md:text-sm text-lg font-medium flex items-center gap-x-1 leading-tight w-fit text-gray-200 hover:text-white">
            <!-- at icon @  -->
            ...
            <!-- username  -->
            <h3 class="truncate tracking-tight">
              {{ material.name }}
            </h3>
          </div>
          <!-- time  -->
          <div
            class="time text-gray-300 hover:text-white flex items-center justify-between indent-4 pl-1.5 max-md:text-xs text-xs">
            <p class="w-fit ">
              {{ material.created_at }}
            </p>
        ...
        <div
          class="relative max-md:order-first md:max-w-[22rem] lg:max-w-[20rem] xl:max-w-[24rem] w-full md:self-center flex-shrink-0 aspect-[21/9] overflow-hidden rounded-xl transition-all duration-300">
          <img :src="`.././${material.image_src}`"
            alt=""
            class="object-cover w-full h-full xl:group-hover:scale-110 xl:group-hover:brightness-105 xl:group-hover:-rotate-2 transition-all duration-300">

          <div class="absolute bottom-2 right-2 flex items-center gap-2">
            <!-- is responded post to request ?  -->
            <div
              class="request-responded-post p-1.5 grid place-content-center bg-gray-50/50 backdrop-blur-md rounded-xl">
              <svg viewBox="0 0 24 24" class="w-5 h-5 flex-shrink-0" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div class="document-type p-1.5 grid place-content-center bg-gray-50/50 backdrop-blur-md rounded-xl">
              <svg viewBox="0 0 24 24" class="w-5 h-5 flex-shrink-0" fill="none" xmlns="http://www.w3.org/2000/svg">
                fill="currentColor" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round">
                </g>
                <g id="SVGRepo_iconCarrier">
                  <path d="M8.78355 21.9999C7.09836 21.2478 5.70641 20.0758 4.7915 18.5068" stroke="#1C274C"
    ...
    <div class="text-sm font-medium flex flex-wrap gap-3  my-6">
      <span class="rounded-xl bg-gray-800 text-white px-2.5 py-1 ">
        Document-Type : {{ material.category }}</span>
      <span class="rounded-xl bg-gray-800 text-white px-2.5 py-1 ">
        Format : {{ material.format }}</span>
      <span class="rounded-full border border-gray-300 px-2.5 py-1 ">#
        {{ material.education_level}}</span>
      <span class="rounded-full border border-gray-300 px-2.5 py-1  ">#
        {{ material.class_faculty }}</span>
      <span class="rounded-full border border-gray-300 px-2.5 py-1 ">#
        {{ material.semester }}</span>
        
    </div>
    <div class="flex flex-col md:flex-row gap-4 my-6">
      <!-- author/source/credits  -->
      <div class="flex items-center gap-3.5 p-3.5 rounded-full bg-gray-100">
        <span class="grid place-content-center w-10 h-10 shrink-0 rounded-full bg-white">
          <svg class="h-8 w-8 flex-shrink-0" viewBox="0 0 400 400" fill="none" xmlns="http://www.w3.org/2000/svg">
            <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
        ...
        <div class="flex flex-col flex-grow">
          <h3 class="text-sm font-medium ">{{ material.author }}</h3>
          <div class="mt-auto ">
            <span class="pr-1.5 text-xs leading-none text-gray-500">Author</span>
            <span class="px-1.5 border-l border-gray-400 text-xs leading-none text-gray-500">Source</span>
            <span class="px-1.5 border-l border-gray-400 text-xs leading-none text-gray-500">Credits</span>
          </div>
        </div>
      </div>
    </div>
    <!-- pdf section  -->
    <iframe :src="`.././${material.file_src}`" width="100%" height="600px"></iframe>
    </section>
</template>
